...
//************DEBUG SHIZ*************
#define START 17 //start pin for debugging Connect to 3.5V on PSX
const int STARTVALUE = 500; // Must see at least this value to start for debug
float Sout = 0.0;
int Sin = 0;
//***********************************
...
void loop() {
  ...
  Serial.println("looping");
    if (readInput() <= STARTVALUE){
    Serial.println(F("Loop runing startup?"));
    STARTUP();
  }
  else if (readInput() >= STARTVALUE){
    Serial.println(F("Loop continuing"));
  }
  //***********************************  
